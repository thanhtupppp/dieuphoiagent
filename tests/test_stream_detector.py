...
def test_check_terminal_signal():
    ...
    is_term, label = check_terminal_signal(
        "Created branch ai-agent/feature and PR "
        "https://github.com/thanhtupppp/repo/pull/8 with commit 9d7e03b",
        "chatgpt",
    )
    assert is_term is True
    assert label == "COMMITTED"
...
async def test_wait_for_completion_terminal_tag_immediate():
    """Verify terminal tags finish on text stability without waiting for timeout."""
