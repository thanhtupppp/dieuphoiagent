...
def build_ui():
    ...
    # Connect FSM logging to LogStreamer
    ...
    with ui.header().classes("bg-slate-900 text-white p-4 flex justify-between items-center shadow-md"):
        ...
        with ui.row().classes("items-center gap-3"):
            cdp_badge = ui.badge("CDP: Port 9222", color="emerald").classes("px-3 py-1 text-xs font-mono")
    ...
    with ui.row().classes("w-full p-4 gap-6 items-start"):
        ...
        with ui.column().classes("w-6/12 bg-white p-5 rounded-xl border border-slate-200 shadow-sm gap-4 flex-1"):
            ...
            with ui.tabs().classes("w-full border-b border-slate-200 text-slate-600") as tabs:
    ...
    # Preview Modal Setup for Step-by-Step Approval
    modal = PreviewModal(
