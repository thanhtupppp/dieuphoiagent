...
class OrchestratorFSM:
    ...
    async def resume_from_checkpoint(self, checkpoint: Optional[TaskCheckpoint] = None):
        ...
        self.loop_count = max(0, cp.loop_count - 1 if cp.next_target_agent == "chatgpt" and cp.loop_count > 0 else cp.loop_count)
        ...
        self.log("system", f"Khôi phục tiến trình từ Checkpoint: Vòng lặp {cp.loop_count}, mục tiêu tiếp theo: {cp.next_target_agent.upper()}")
        if self.pr_url:
        ...
        asyncio.create_task(self._run_loop(start_agent=cp.next_target_agent, start_payload=cp.next_prompt_payload))

    async def retry_step(self):
        self.log("system", "Thực hiện thử lại bước hiện tại...")
        await self.resume_from_checkpoint()

    async def reconcile_and_resume(self, repo: str, branch: str, goal: str, max_loops: int = 5, auto_mode: bool = True):
        ...
        save_checkpoint(cp)
        self.log("system", f"Đồng bộ thành công: Trạng thái [{cp.status_label}], Agent tiếp theo: [{cp.next_target_agent.upper()}].")
        if cp.pr_url:
    ...
    async def _run_loop(self, start_agent: str = "perplexity", start_payload: Optional[str] = None):
        try:
            ...
            # Initial prompt if starting clean
            ...
            while self.is_running and self.loop_count < self.max_loops:
                if current_turn == "perplexity":
                    ...
                    # Persist checkpoint after Perplexity success
                ...
                if current_turn == "chatgpt":
                    ...
                    await self.detector.send_prompt(self.connector.chatgpt_tab, next_prompt, "chatgpt")
                    ...
                    if c_result.payload:
                        self.feature_branch = getattr(c_result.payload, "branch", self.feature_branch) or self.feature_branch
                        self.commit_sha = getattr(c_result.payload, "commit_sha", self.commit_sha) or self.commit_sha
                        self.pr_url = getattr(c_result.payload, "pr_url", self.pr_url) or self.pr_url
                    ...
                    # Persist checkpoint after ChatGPT success
                    save_checkpoint(TaskCheckpoint(
                        repo=self.current_repo,
                        branch=self.current_branch,
                        goal=self.current_goal,
            ...
            self.log("system", f"Sự cố thực thi: {err_msg}. Đã lưu trạng thái phục hồi (Checkpoint).")
            self.log("system", "Hệ thống đang chờ lệnh cứu hộ: bạn có thể bấm 'Thử lại bước' (Retry) hoặc 'Tiếp tục từ Checkpoint' (Resume).")
